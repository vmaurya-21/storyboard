...
describe('storyHelpers', () => {
  it('classifies LinkedIn and external/internal domains', () => {
    ...
    expect(detectStorySource('https://whitecase.com/a')).toEqual({ source: 'internal', domain: 'whitecase.com' });
    expect(detectStorySource('https://external.whitecase.com/a')).toEqual({ source: 'external', domain: 'external.whitecase.com' });
